import { IoClose, IoOptionsOutline, IoConstruct, IoWarningOutline } from "react-icons/io5";
...
const VoiceSettings = ({ isOpen, onClose, voices, activeVoice, onVoiceChange, isAutoRead, onToggleAutoRead, customPrompt, setCustomPrompt, ttsRate, setTtsRate, ttsPitch, setTtsPitch, appTheme, setAppTheme, sttLang, setSttLang }) => {
  ...
  const handleFactoryReset = () => {
    if (confirm("⚠️ WARNING: This will clear ALL settings, history, and custom personas. Continue?")) {
    ...
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/70 backdrop-blur-sm animate-fade-in p-4" onClick={onClose}>
      <div className="bg-slate-900 border border-slate-700 rounded-2xl w-full max-w-md p-6 shadow-2xl relative max-h-[90vh] overflow-y-auto custom-scrollbar" onClick={e => e.stopPropagation()}>
        ...
        <h2 className="text-lg font-bold text-white mb-6 flex items-center gap-2">
          {activeTab === "prefs" ? <IoOptionsOutline className="text-indigo-400"/> : <IoConstruct className="text-indigo-400"/>} App Settings
        </h2>
        ...
        {activeTab === "prefs" ? (
          <div className="space-y-6">
            ...
            <div className="space-y-4 border-b border-slate-800 pb-6">
              <h3 className="text-xs font-bold text-indigo-400 uppercase tracking-wider">Visual Appearance</h3>
              
              <div className="space-y-2">
                <label className="text-xs font-bold text-slate-500 uppercase tracking-wider">App Theme</label>
                <select value={appTheme} onChange={(e) => setAppTheme(e.target.value)} className="w-full bg-slate-800 text-white p-3 rounded-xl border border-slate-700 focus:border-indigo-500 outline-none text-sm appearance-none cursor-pointer">
          ...
          <div className="space-y-6 animate-fade-in">
             ...
             {customPrompt && (<button onClick={() => setCustomPrompt("")} className="text-xs text-indigo-400 hover:text-indigo-300 font-medium">Clear Custom Prompt</button>)}
             
             <div className="border-t border-red-900/50 pt-6 mt-6">
                <h3 className="text-xs font-bold text-red-500 uppercase tracking-wider mb-2 flex items-center gap-2"><IoWarningOutline size={16}/> Danger Zone</h3>
