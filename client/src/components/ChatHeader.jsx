...
const ChatHeader = ({ currentPersona, onExportMarkdown, onExportJSON, onCopyAll, onImportJSON, onImportBackup, onOpenAnalytics, isFocusMode, onToggleFocus, hasCustomPrompt, onToggleSearch, isSearchOpen, onToggleBookmarks, onToggleDesktopSidebar }) => {
  ...
  return (
    <div className={`absolute top-0 left-0 right-0 p-4 flex justify-between items-start pointer-events-none z-30 transition-all duration-500 print:hidden ${isFocusMode ? "-translate-y-full opacity-0" : "translate-y-0 opacity-100"}`}>
      <div className="pointer-events-auto flex items-center gap-3">
         <button onClick={onToggleDesktopSidebar} className="hidden md:flex p-1.5 text-slate-400 hover:text-white hover:bg-slate-800 rounded-lg transition-colors border border-transparent hover:border-slate-700 backdrop-blur" title="Toggle Sidebar"><IoMenuOutline size={20} /></button>
         <PersonaBadge persona={currentPersona} />
         {hasCustomPrompt && (
            <div className="flex items-center gap-1 text-[10px] font-bold text-amber-500 bg-amber-500/10 px-2 py-1 rounded-full border border-amber-500/20 shadow-sm backdrop-blur" title="Custom System Prompt Active"><IoWarningOutline size={12} /> <span className="hidden sm:inline">Custom Rule</span></div>
         )}
      </div>
      <div className="pointer-events-auto flex items-center gap-2">
        ...
        <div className="relative">
          ...
          <ExportMenu isOpen={isMenuOpen} onExportMarkdown={() => { onExportMarkdown(); setIsMenuOpen(false); }} onExportJSON={() => { onExportJSON(); setIsMenuOpen(false); }} onCopyAll={() => { onCopyAll(); setIsMenuOpen(false); }} onImportJSON={(file) => { onImportJSON(file); setIsMenuOpen(false); }} onImportBackup={(file) => { onImportBackup(file); setIsMenuOpen(false); }} onOpenAnalytics={() => { onOpenAnalytics(); setIsMenuOpen(false); }} />
