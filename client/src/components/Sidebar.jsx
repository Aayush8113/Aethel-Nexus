    ...
    return (
      <div className="mb-6">
        <h3 className="text-[10px] font-bold text-slate-500 uppercase tracking-widest mb-3 px-2 flex items-center gap-2">
          {title === "Pinned" && (
            <IoPin className="text-indigo-400" size={10} />
          )}
          {title}
        </h3>
        <div className="space-y-1">
          {list.map((chat) => (
            <div key={chat._id} className="relative group">
              ...
              >
                <div
                  className={`transition-colors ${chat.isPinned ? "text-indigo-400" : "text-slate-500 group-hover:text-slate-400"}`}
                >
                  {chat.isPinned ? (
                    <IoPin size={14} />
                  ) : (
                ...
                <span className="pr-12 text-sm font-medium truncate">
                  {chat.title}
                </span>
              </button>
      ...
      <div className="flex flex-col h-full p-4">
        ...
        <div className="pt-4 mt-4 space-y-1 border-t border-slate-800/50">
          ...
          {chats.length > 0 && (
            ...
            >
              <div className="transition-colors text-slate-500 group-hover:text-red-400">
                <IoTrashOutline size={16} />
              </div>
              <span className="text-xs font-medium text-slate-500 group-hover:text-red-400">
                Clear History
              </span>
            </button>
          )}
          ...
          >
            ...
            <div className="text-left">
              <p className="text-xs font-medium transition-colors text-slate-300 group-hover:text-white">
                Persona
              </p>
            </div>
          </button>
