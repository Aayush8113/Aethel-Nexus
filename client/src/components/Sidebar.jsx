...
import PingIndicator from "./PingIndicator"; // New
...
const Sidebar = ({ chats, activeChatId, onSelectChat, onNewChat, onDeleteChat, onTogglePin, onClearAll, onOpenSettings, onOpenPersonas, isOpen, onClose, isLoading, isInstallable, installApp }) => {
  ...
  useEffect(() => {
    const handleKeyDown = (e) => {
      if (e.key === "/" && !e.ctrlKey && !e.metaKey && !e.altKey) { e.preventDefault(); searchInputRef.current?.focus(); }
    };
        ...
        <div className="space-y-1">
          {list.map((chat) => (
            <div key={chat._id} className="relative group">
              <button onClick={() => onSelectChat(chat._id)} className={`w-full text-left px-3 py-2.5 rounded-lg flex items-center gap-3 transition-all ${activeChatId === chat._id ? "bg-slate-800 text-indigo-300 border border-slate-700 shadow-sm" : "text-slate-400 hover:bg-slate-800/50 hover:text-slate-200"}`}>
              ...
              <div className="absolute right-2 top-1/2 -translate-y-1/2 flex items-center gap-1 opacity-0 group-hover:opacity-100 transition-all z-10 bg-slate-900/90 backdrop-blur rounded px-1 py-0.5 border border-slate-800">
                 <button onClick={(e) => { e.stopPropagation(); onTogglePin(chat._id); }} className="p-1.5 text-slate-500 hover:text-indigo-400"><IoPin size={13} /></button>
                 <button onClick={(e) => { e.stopPropagation(); onDeleteChat(chat._id); }} className="p-1.5 text-slate-500 hover:text-red-400"><IoTrashOutline size={13} /></button>
              </div>
            </div>
          ))}
        </div>
  ...
  return (
    <div {...swipeHandlers} className={`fixed inset-y-0 left-0 z-30 w-72 bg-slate-900/95 backdrop-blur-2xl border-r border-slate-800 transform transition-transform duration-300 ease-in-out ${isOpen ? "translate-x-0" : "-translate-x-full"} md:relative md:translate-x-0 flex flex-col h-full`}>
      <div className="flex flex-col h-full p-4">
        <div className="flex items-center gap-3 mb-6 px-2">
          <div className="w-8 h-8 rounded-lg bg-gradient-to-tr from-indigo-500 to-purple-600 flex items-center justify-center"><IoCubeOutline size={20} className="text-white" /></div>
        ...
        <div className="flex-1 pr-1 -mr-2 overflow-y-auto custom-scrollbar">
          {isLoading ? (<div className="px-2 mt-2 space-y-3"><div className="w-full h-10 rounded-lg bg-slate-800/50 animate-pulse"></div></div>) : chats.length === 0 ? (<div className="flex flex-col items-center justify-center h-40 px-4 mt-8 text-center opacity-50"><IoChatboxOutline size={32} className="mb-2 text-slate-600" /><p className="text-sm italic text-slate-500">No history found.</p></div>) : (<> {renderChatList(pinnedChats, "Pinned")} {renderChatList(recentChats, "Recent")} </>)}
        </div>
        <div className="pt-4 mt-4 space-y-1 border-t border-slate-800/50">
          {isInstallable && (<button onClick={installApp} className="flex items-center justify-center gap-2 w-full px-4 py-2 mb-3 bg-gradient-to-r from-indigo-600 to-purple-600 text-white rounded-lg shadow-lg font-bold text-xs"><IoCubeOutline /><span>Install App</span></button>)}
