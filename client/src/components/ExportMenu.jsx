import { IoDocumentTextOutline, IoCodeSlashOutline, IoCopyOutline, IoPushOutline, IoSaveOutline, IoPieChartOutline } from "react-icons/io5";
...
const ExportMenu = ({ isOpen, onExportMarkdown, onExportJSON, onCopyAll, onImportJSON, onImportBackup, onOpenAnalytics }) => {
  ...
  return (
    <div className="absolute top-12 right-0 bg-slate-900 border border-slate-700 rounded-xl shadow-2xl p-1 w-64 z-50 animate-fade-in origin-top-right">
      ...
      {/* Day 27: Analytics Button */}
      <button onClick={onOpenAnalytics} className="flex items-center gap-3 w-full px-3 py-2.5 text-sm text-slate-300 hover:bg-slate-800 hover:text-white rounded-lg transition-colors">
        <IoPieChartOutline size={16} className="text-purple-400" /><span>View Analytics</span>
      </button>
